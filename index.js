...
function renderListas(l){var cont=document.getElementById('listaContenedor');var arr=Object.values(l);if(arr.length===0){cont.innerHTML='<p style="color:#a0aec0;font-size:14px">No hay listas creadas</p>';return}var h='';for(var i=0;i<arr.length;i++){var li=arr[i];var cn=li.colegio_nombre||li.colegioNombre||'';var cu=li.curso||'';var se=li.seccion||'';var fc=li.fecha_creacion||li.fechaCreacion||'';var ur=li.url_registro||li.urlRegistro||'';var total=0;for(var j=0;j<registrosData.length;j++){if(registrosData[j].lista_id===li.id||registrosData[j].listaId===li.id)total++}h+='<div class="lista-card"><h4>📋 '+cn+'</h4><p style="color:#4a5568;font-size:14px">'+cu+' '+se+'</p><div class="info">📊 <strong>'+total+'</strong> registros | 📅 '+new Date(fc).toLocaleDateString("es-CL")+'</div><div class="acciones"><button class="btn btn-verde btn-chico" onclick="verQR(\\''+li.id+'\\')">📱 Ver QR</button><button class="btn btn-chico" onclick="descargarExcel(\\''+li.id+'\\')">📥 Excel</button><button class="btn btn-rojo btn-chico" onclick="descargarPDF(\\''+li.id+'\\')">📄 PDF</button><button class="btn btn-gris btn-chico" onclick="copiarEnlace(\\''+ur+'\\')">🔗 Link</button><button class="btn btn-rojo btn-chico" onclick="eliminarLista(\\''+li.id+'\\')">🗑️</button></div></div>'}cont.innerHTML=h}function mostrarModalColegio(){document.getElementById('modalColegio').style.display='flex'}
...
async function verQR(id){try{var r=await fetch('/api/lista/'+id,{headers:{'Authorization':'Bearer '+token}});var d=await r.json();if(d.lista){var qr=d.lista.qr_code||d.lista.qrCode||'';var url=d.lista.url_registro||d.lista.urlRegistro||'';document.getElementById('qrImage').src=qr;document.getElementById('linkRegistro').textContent=url;document.getElementById('modalQR').style.display='flex'}}catch(err){msg('❌ Error','error')}}async function eliminarLista(id){if(!confirm('¿Eliminar esta lista y todos sus registros?'))return;try{var r=await fetch('/api/eliminar-lista/'+id,{method:'DELETE',headers:{'Authorization':'Bearer '+token}});if(r.ok){msg('✅ Lista eliminada','exito');cargarDatos()}else{msg('❌ Error al eliminar','error')}}catch(err){msg('❌ Error','error')}}async function eliminarLista(id){if(!confirm('¿Eliminar esta lista y todos sus registros?'))return;try{var r=await fetch('/api/eliminar-lista/'+id,{method:'DELETE',headers:{'Authorization':'Bearer '+token}});if(r.ok){msg('✅ Lista eliminada','exito');cargarDatos()}else{msg('❌ Error al eliminar','error')}}catch(err){msg('❌ Error','error')}}function copiarEnlace(url){navigator.clipboard.writeText(url).then(function(){msg('📋 Enlace copiado','exito')})}
...
app.get('/r/:code', async (req, res) => {
  ...
  if (!lista) {
    return res.send('<div style="text-align:center;margin-top:100px;font-family:Arial"><h1>❌ Lista no encontrada</h1></div>');
  }
  
  const colegioNombre = lista.colegio_nombre || '';
  const curso = lista.curso || '';
  const seccion = lista.seccion || '';
  
  res.send(`<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Registro - ${colegioNombre}</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
<div class="header"><h2>📋 Registro de Apoderados</h2><h3>${colegioNombre}</h3><p style="font-size:16px;margin-top:8px">${curso} ${seccion}</p></div>
<div id="mensaje" class="mensaje"></div>
...
<div class="form-group"><label class="required">Correo</label><input type="email" id="email" required></div>
<div class="form-group"><label class="required">Teléfono</label><input type="tel" id="telefono" required></div>
...
<div id="pantallaExito" style="display:none;text-align:center"><div style="background:white;padding:40px;border-radius:15px"><div style="font-size:80px">✅</div><h1 style="color:#276749" id="tituloExito">¡Registro Exitoso!</h1><p id="mensajeExito">Gracias por registrarse.</p></div></div>
<script>
var codigo='${req.params.code}';var listaId='${lista.id}';var registroExistente=null;
...
async function buscarRegistro(){var rut=document.getElementById('rutBuscar').value;if(!rut)return msg('❌ Ingrese RUT','error');try{var r=await fetch('/api/buscar/'+listaId+'/'+encodeURIComponent(rut));var d=await r.json();if(d.existe){registroExistente=d.registro;document.getElementById('panelBuscar').style.display='none';document.getElementById('formRegistro').style.display='block';document.getElementById('modoEdicion').value='editar';document.getElementById('registroId').value=d.registro.id;document.getElementById('btnSubmit').textContent='🔄 Actualizar Datos';var reg=d.registro;document.getElementById('nombresApoderado').value=reg.nombres_apoderado||reg.nombresApoderado||'';document.getElementById('apellidoPaternoApoderado').value=reg.apellido_paterno_apoderado||reg.apellidoPaternoApoderado||'';document.getElementById('apellidoMaternoApoderado').value=reg.apellido_materno_apoderado||reg.apellidoMaternoApoderado||'';document.getElementById('rutApoderado').value=reg.rut_apoderado||reg.rutApoderado||'';document.getElementById('email').value=reg.email||'';document.getElementById('telefono').value=reg.telefono||'';document.getElementById('direccion').value=reg.direccion||'';document.getElementById('comuna').value=reg.comuna||'';document.getElementById('relacion').value=reg.relacion||'';document.getElementById('nombresEstudiante').value=reg.nombres_estudiante||reg.nombresEstudiante||'';document.getElementById('apellidoPaternoEstudiante').value=reg.apellido_paterno_estudiante||reg.apellidoPaternoEstudiante||'';document.getElementById('apellidoMaternoEstudiante').value=reg.apellido_materno_estudiante||reg.apellidoMaternoEstudiante||'';document.getElementById('rutEstudiante').value=reg.rut_estudiante||reg.rutEstudiante||'';document.getElementById('avisoExistente').style.display='block';document.getElementById('avisoExistente').innerHTML='📝 <strong>Ya estás registrado.</strong> Puedes actualizar tus datos.';msg('📝 Datos cargados.','info')}else{registroExistente=null;document.getElementById('panelBuscar').style.display='none';document.getElementById('formRegistro').style.display='block';document.getElementById('modoEdicion').value='nuevo';document.getElementById('btnSubmit').textContent='✅ Confirmar Registro';document.getElementById('formRegistro').reset();msg('🆕 Complete sus datos.','info')}}catch(err){msg('❌ Error','error')}}
document.getElementById('formRegistro').addEventListener('submit',async function(e){e.preventDefault();var btn=document.getElementById('btnSubmit');btn.disabled=true;btn.textContent='...';var datos={nombresApoderado:document.getElementById('nombresApoderado').value,apellidoPaternoApoderado:document.getElementById('apellidoPaternoApoderado').value,apellidoMaternoApoderado:document.getElementById('apellidoMaternoApoderado').value,rutApoderado:document.getElementById('rutApoderado').value,email:document.getElementById('email').value,telefono:document.getElementById('telefono').value,direccion:document.getElementById('direccion').value,comuna:document.getElementById('comuna').value,relacion:document.getElementById('relacion').value,nombresEstudiante:document.getElementById('nombresEstudiante').value,apellidoPaternoEstudiante:document.getElementById('apellidoPaternoEstudiante').value,apellidoMaternoEstudiante:document.getElementById('apellidoMaternoEstudiante').value,rutEstudiante:document.getElementById('rutEstudiante').value,modo:document.getElementById('modoEdicion').value,registroId:document.getElementById('registroId').value};try{var r=await fetch('/api/registrar/'+codigo,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(datos)});var d=await r.json();if(r.ok&&d.success){document.getElementById('formContainer').style.display='none';document.getElementById('pantallaExito').style.display='block';if(datos.modo==='editar'){document.getElementById('tituloExito').textContent='✅ ¡Actualizado!';document.getElementById('mensajeExito').textContent='Datos actualizados.'}}else{msg('❌ '+(d.errores||['Error']).join('<br>'),'error')}}catch(err){msg('❌ Error de conexión','error')}finally{btn.disabled=false;btn.textContent=datos.modo==='editar'?'🔄 Actualizar':'✅ Confirmar Registro'}});
</script></body></html>`);
});
...
app.delete('/api/eliminar-lista/:id', autenticar, async (req, res) => {
  try {
    await supabase.from('registros').delete().eq('lista_id', req.params.id);
    const { error } = await supabase.from('listas').delete().eq('id', req.params.id).eq('usuario_id', req.usuarioId);
    if (error) throw error;
